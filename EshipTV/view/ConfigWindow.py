...
class ConfigWindow(QDialog):
    ...
    def showEvent(self, event):
        try:
            self.textViewUser.setText(globals.user['user'])
            self.textViewPassword.setText(globals.user['password'])
            self.textViewURL.setText(globals.user['url'])
            self.textViewTimeout.setText(str(globals.user['timeout']))
        except Exception:
            pass
    ...
    def _on_button_click(self, checked):
        try:
            with open(JSON_FILE, "w+") as json_file:
                globals.user['user'] = self.textViewUser.text()
                globals.user['password'] = self.textViewPassword.text()
                globals.user['url'] = self.__format_url(self.textViewURL.text())
                if self.textViewTimeout.text() == '':
                    globals.user['timeout'] = float(0)
                else:
                    globals.user['timeout'] = float(self.textViewTimeout.text())
