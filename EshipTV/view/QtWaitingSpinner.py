...
class QtWaitingSpinner(QLabel):
    mColor = QColor(Qt.gray)
    mRoundness = 100.0
    mMinimumTrailOpacity = 100
    ...
    def __init__(self, centerOnParent=True, disableParentWhenSpinning=False, *args, **kwargs):
        QWidget.__init__(self, *args, **kwargs)
        self.setWindowFlags(Qt.FramelessWindowHint)
        self.setAttribute(Qt.WA_TranslucentBackground)
        self.setStyleSheet("background:transparent;")
        movie = QMovie("static/ajax-loader.gif")
        self.setMovie(movie)
        # self.show()
        movie.start()
